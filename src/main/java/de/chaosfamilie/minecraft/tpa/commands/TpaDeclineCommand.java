...
public class TpaDeclineCommand {
    ...
    public static LiteralCommandNode<CommandSourceStack> constructCommand() {
        return Commands.literal("tpadecline")
                .requires(ctx -> ctx.getExecutor() instanceof Player)
                .then((Commands
                        .argument("player", StringArgumentType.string()))
                        .suggests(NetworkPlayerSuggest::suggestPlayers)
                        .executes(TpaDeclineCommand::execute))
                .build();
    }
    ...
    private static int execute(final CommandContext<CommandSourceStack> ctx) {
        ...
        requester.sendMessage(mm.deserialize("<gold>" + player.getName() + "<red> Declined your teleport request"));
        requester.playSound(Sound.sound(Key.key("minecraft:entity.experience_orb.pickup"), Sound.Source.MASTER, 100, 0));
